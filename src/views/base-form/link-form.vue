...
export default{
      ...
      return {
        formData:{
          list:[
            ...
            {type:"input",field:"__inputtiaojian",title:"A为特定值时，B只能为特定值"},
            {
              type: "input",
              field: "__inputRules",
              title: "A为特定值时,则非必填",
	...
	mounted() {
		...
		this.formData.list.push( {//测试 dis   
		        type: "select",
		        field: "__input5",
		        title: "多选一",
		        opt:[
    ...
    watch:{
      "formData.data": {
          handler(newVal) {
            ...
            if (newVal.__select == "1") {
                    this._set(this.formData, "__input", {show:false});
            } else{
                    this._set(this.formData, "__input", {show:true});
            }

            //2 A 为特定值时，B 只能为特定范围内的值 (或不能为某些值)  --- 多选框
            if (newVal.__select == "1") {
                    //为了防止改field的值是不存在的,所以将将不存在的值重置为”“
                    if (this.formData.data.__selecttiaojian == "2") {
                      this.formData.data.__selecttiaojian = "";
                    }
                    this._set(this.formData, "__selecttiaojian", 
                      {
                        opt:[
                        {
                          text: "特定范围",
                    ...
                    this._set(this.formData, "__selecttiaojian", {opt:[{text:"特定范围",value:"1"},{text:"普通值2",value:"2"}]});
            }

            //3 A 为特定值时，B 只能为特定值                       ---输入框
            if (newVal.__select == "1") {
              this.formData.data.__inputtiaojian = "特定值";
            }

            //4 有条件的校验
            if (newVal.__select == "1") {
              this._set(this.formData, "__inputRules", {rules:[]});
            } else {
              let __input2Arr = [
                { required: true, message: "请输入", trigger: "blur" },
                { validator: this.$check.demo, trigger: "blur" },
              ];
              this._set(this.formData, "__inputRules",{rules:__input2Arr});
            }
            ...
            if (this.formData.data.__input3 || this.formData.data.__input5) {
              this._set(this.formData, "__input4",{disabled:true})
            } else {
              this._set(this.formData, "__input4",{disabled:false})
            }
